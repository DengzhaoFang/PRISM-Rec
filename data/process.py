...
def parse_args():
    ...
    parser.add_argument('--embed_mode', type=str, default='tiger',
                       choices=['tiger', 'hid-vae'],
                       help='Embedding mode: tiger (concat all) or hid-vae (separate categories)')
    ...
    parser.add_argument('--max_tags', type=int, default=5,
                       help='Maximum number of category tags for hid-vae mode')
...
def generate_hidvae_embeddings(item_info, model, max_tags, logger):
    """HID-VAE mode: Separate embeddings for categories and other attributes
...
def generate_all_item_embeddings(args, output_path, logger):
    ...
    tag_info = None  # Only used for hid-vae mode
    if args.embed_mode == 'tiger':
        item_embeddings = generate_tiger_embeddings(item_info, model, logger)
    elif args.embed_mode == 'hid-vae':
        item_embeddings, tag_info = generate_hidvae_embeddings(item_info, model, args.max_tags, logger)
    else:
    ...
    # Save tag information for hid-vae mode
    if args.embed_mode == 'hid-vae' and tag_info is not None:
...
# --- 3b. Generate Filtered Embeddings (for Recommender Training) ---
...
def main():
    ...
    logger.info("="*80)
    logger.info("AMAZON DATASET PREPROCESSING PIPELINE (TWO-STAGE: ALL ITEMS + 5-CORE FILTERING)")
    logger.info("="*80)
    logger.info(f"Dataset: {args.dataset}")
    logger.info(f"Review path: {args.review_path}")
    logger.info(f"Meta path: {args.meta_path}")
    logger.info(f"Output dir: {args.output_dir}")
    logger.info(f"Min interactions (5-core): {args.min_interactions}")
    ...
    logger.info("          then apply 5-core filter only for interaction data (recommender training)")
    
    # Step 1: Convert raw data
    output_path = convert_raw_to_json(args, logger)
    
    # Step 2: Generate embeddings for ALL items (before filtering, for tokenizer training)
    item_emb_all_df, itemID_mapping_all = generate_all_item_embeddings(args, output_path, logger)
    
    # Step 3: Preprocess interactions with 5-core filtering (for recommender training)
    itemID_mapping_filtered = preprocess_interactions(args, output_path, logger)
    
    # Step 4: Generate filtered embeddings (optional, for recommender if needed)
    ...
    logger.info(f"    - user_mapping.npy (user ID mapping, 5-core filtered)")
    logger.info(f"    - item_mapping.npy (item ID mapping, 5-core filtered, for recommender)")
    logger.info(f"    - item_mapping_all.npy (item ID mapping, ALL items, for tokenizer)")
    logger.info(f"")
    logger.info(f"  Interaction data (5-core filtered, for recommender training):")
    ...
    logger.info(f"    - item_emb.parquet (5-core filtered items, for recommender training)")
    
    if args.embed_mode == 'hid-vae':
        logger.info(f"")
        logger.info(f"  Tag files (hid-vae mode only):")
    ...
    logger.info("✓ Log file contains complete processing details")
    logger.info("")
    logger.info("="*80)
    logger.info("USAGE GUIDE:")
    logger.info("="*80)
    logger.info("1. For TOKENIZER training (RQVAE/RQ-KMeans):")
    logger.info("   Use: item_emb_all.parquet (contains ALL items)")
    logger.info("")
    logger.info("2. For RECOMMENDER training:")
    logger.info("   Use: train.parquet + valid.parquet + test.parquet (5-core filtered)")
    logger.info("   And: item_mapping.npy (5-core filtered item mapping)")
    logger.info("="*80)
