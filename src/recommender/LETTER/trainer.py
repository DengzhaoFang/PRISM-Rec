...
from .trie import Trie, get_prefix_allowed_tokens_fn
...
class Trainer:
    ...
    def __init__(
        ...
        # Initialize Trie for constrained generation
        logger.info("Initializing Trie for constrained generation...")
        # Get semantic mapping from dataset (assuming all loaders share the same mapper)
        semantic_mapper = train_loader.dataset.semantic_mapper
        # Extract all valid code sequences
        valid_sequences = list(semantic_mapper.item_to_codes.values())
        self.trie = Trie(valid_sequences)
        logger.info(f"Trie initialized with {len(valid_sequences)} sequences")
    ...
    def train_epoch(self) -> float:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            loss, _ = self.model(
                ...
                labels=labels,
                temperature=getattr(self.training_config, 'temperature', 1.0)
            )
    ...
    def evaluate(self, data_loader, split_name: str = "Valid") -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in progress_bar:
                ...
                prefix_allowed_tokens_fn = get_prefix_allowed_tokens_fn(self.trie)
