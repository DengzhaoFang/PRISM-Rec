...
class Trainer:
    ...
    def __init__(
        ...
        # Store semantic mapper for EAGER decoding
    ...
    def train_epoch(self) -> float:
        ...
        for batch in progress_bar:
            ...
            if 'history_item_ids' in batch:
                # EAGER Forward
                # Generate attention mask for history items
                history_item_ids = batch['history_item_ids']
                pad_token_id = self.config['model'].pad_token_id
                attention_mask = (history_item_ids != pad_token_id).long()
                
                loss, loss_dict = self.model(
                    history_item_ids=batch['history_item_ids'],
                    target_behavior_codes=batch['target_behavior_codes'],
                    target_semantic_codes=batch['target_semantic_codes'],
                    target_item_id=batch['target_item_id'],
                    attention_mask=attention_mask,
                    lambda_1=self.training_config.lambda_1,
                    lambda_2=self.training_config.lambda_2,
                    mask_ratio_recon=self.training_config.mask_ratio_recon,
                    mask_ratio_recog=self.training_config.mask_ratio_recog,
                    num_negatives=getattr(self.training_config, 'num_negatives', 64)
                )
                ...
                if self.global_step % self.training_config.log_every_n_steps == 0:
                    logger.info(
                        ...
                        f"gen={loss_dict['loss_gen']:.4f}, "
                        f"con={loss_dict['loss_con']:.4f}, "
                        f"stt={loss_dict['loss_stt']:.4f}"
                    )
            else:
                # TIGER Forward
                ...
                # Generate attention mask if missing
                if 'attention_mask' in batch:
                    attention_mask = batch['attention_mask']
                else:
                    attention_mask = (input_ids != self.config['model'].pad_token_id).long()
            ...
            # Log periodically (for TIGER, EAGER logged above)
            if self.global_step % self.training_config.log_every_n_steps == 0 and 'history_item_ids' not in batch:
                logger.info(
                    f"Step {self.global_step}: loss={loss.item():.4f}, "
    ...
    def evaluate(self, data_loader, split_name: str = "Valid") -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in progress_bar:
                ...
                # max_length = num_code_layers + 1 (EOS) + 1 (decoder start token that will be removed)
                # Target format: [code0, code1, ..., EOS]
                batch = {k: v.to(self.device) for k, v in batch.items() if isinstance(v, torch.Tensor)}
                max_gen_length = self.model.config.num_code_layers + 2  # +1 for EOS, +1 for start token
                
                # Initialize variables for both EAGER and TIGER modes
                item_ids = None
                target_item_ids = None
                
                if 'history_item_ids' in batch:
                    # EAGER Generation with Dual-Stream Ranking
                    input_ids = batch['history_item_ids']
                    attention_mask = (input_ids != self.config['model'].pad_token_id).long()
                    
                    # Get dual-stream predictions with scores
                    behavior_ids, behavior_scores, semantic_ids, semantic_scores = self.model.generate(
                        input_ids=input_ids,
                        attention_mask=attention_mask,
                    ...
                    # Remove start token from generated sequences
                    behavior_ids = behavior_ids[:, 1:]  # (batch_size * num_beams, seq_len)
                    semantic_ids = semantic_ids[:, 1:]
                    
                    # Remove EOS token if present (last position)
                    # Generated: [code0, code1, EOS] -> [code0, code1]
                    # This matches the labels format (which also has EOS removed)
                    if behavior_ids.size(1) > self.config['model'].num_code_layers:
                        behavior_ids = behavior_ids[:, :-1]  # Remove last token (EOS)
                        semantic_ids = semantic_ids[:, :-1]
                    
                    batch_size = input_ids.size(0)
                    num_beams = self.training_config.beam_size
                    seq_len = behavior_ids.size(1)  # Should be num_code_layers
                    
                    # Merge dual-stream candidates with confidence-based ranking
                    # Result shape: (batch_size, num_beams, seq_len)
                    merged_preds = torch.zeros((batch_size, num_beams, seq_len), dtype=torch.long, device=self.device)
                    # NEW: Also store item_ids for fair evaluation
                    merged_item_ids = torch.zeros((batch_size, num_beams), dtype=torch.long, device=self.device)
                    
                    for i in range(batch_size):
                        candidates = []
                        
                        # Add behavior stream candidates
                        for j in range(num_beams):
                            idx = i * num_beams + j
                            codes_tensor = behavior_ids[idx]  # Already a tensor
                            score = behavior_scores[idx].item()
                            
                            # Decode codes to item_id
                            item_id = None
                            if self.semantic_mapper is not None:
                                codes_list = codes_tensor.tolist()
                                item_id = self.semantic_mapper.decode_behavior_codes(codes_list)
                            
                            if item_id is not None:
                                candidates.append({
                                    'item_id': item_id,
                                    'score': score,
                                    'codes_tensor': codes_tensor,
                                    'stream': 'behavior'
                                })
                        
                        # Add semantic stream candidates
                        for j in range(num_beams):
                            idx = i * num_beams + j
                            codes_tensor = semantic_ids[idx]  # Already a tensor
                            score = semantic_scores[idx].item()
                            
                            # Decode codes to item_id
                            item_id = None
                            if self.semantic_mapper is not None:
                                codes_list = codes_tensor.tolist()
                                item_id = self.semantic_mapper.decode_semantic_codes(codes_list)
                        ...
                        # Sort by score (descending - higher log prob is better)
                        ...
                        # Take top num_beams unique candidates
                        top_candidates = unique_candidates[:num_beams]
                        
                        # Fill merged_preds and merged_item_ids for this sample
                        for j, c in enumerate(top_candidates):
                            # CHANGED: Keep original codes (don't convert to behavior codes)
                            # This preserves the stream's original prediction
                            merged_preds[i, j] = c['codes_tensor']
                            # NEW: Store item_id for fair evaluation
                            merged_item_ids[i, j] = c['item_id']
                        
                        # Pad remaining slots if we don't have enough candidates
                        for j in range(len(top_candidates), num_beams):
                            # Use padding token
                            merged_preds[i, j] = self.config['model'].pad_token_id
                            merged_item_ids[i, j] = 0  # Padding item_id
                    
                    preds = merged_preds
                    # Reshape to (batch_size * num_beams, seq_len) for metrics calculator
                    preds = preds.reshape(batch_size * num_beams, seq_len)
                    
                    # Labels include EOS token at the end, remove it for evaluation
                    # Format: [code0, code1, ..., EOS] -> [code0, code1, ...]
                    labels = batch['target_behavior_codes'][:, :-1]  # Remove last token (EOS)
                    
                    # NEW: Get target item_ids for fair evaluation
                    target_item_ids = batch['target_item_id']  # (batch_size,)
                    item_ids = merged_item_ids  # (batch_size, num_beams)
                else:
                    # TIGER Generation
                    input_ids = batch['history']
                    if 'attention_mask' in batch:
                ...
                # Remove start token (only for TIGER, EAGER already removed it)
                if 'history_item_ids' not in batch:
                    preds = preds[:, 1:]
                ...
                # Update metrics
                # For EAGER: pass item_ids for fair dual-stream evaluation
                # For TIGER: item_ids will be None, fallback to code-based comparison
                if 'history_item_ids' in batch:
                    # EAGER mode: use item_id based evaluation
                    ...
                    # TIGER mode: use code-based evaluation
                    self.metrics_calculator.update(
                        preds, labels, self.training_config.beam_size
                    )
