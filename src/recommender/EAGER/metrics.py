...
def calculate_pos_index_by_item_id(
    pred_item_ids: torch.Tensor,
    target_item_ids: torch.Tensor,
    maxk: int = 20
) -> torch.Tensor:
    """Calculate position indices based on item IDs (not codes).
    
    This function checks if the predicted item IDs match the ground truth
    item IDs and returns a boolean tensor indicating matches at each beam position.
    
    This is more fair for dual-stream evaluation as it doesn't require
    converting semantic codes to behavior codes.
    
    Args:
        pred_item_ids: Predicted item IDs, shape (batch_size, maxk)
        target_item_ids: Ground truth item IDs, shape (batch_size,)
        maxk: Maximum k value (beam size)
    ...
    Returns:
        ...
        prediction at each position matches the ground truth
    """
    pred_item_ids = pred_item_ids.detach().cpu()
    target_item_ids = target_item_ids.detach().cpu()
    
    assert pred_item_ids.shape[1] == maxk, f"pred_item_ids.shape[1] = {pred_item_ids.shape[1]} != {maxk}"
    
    batch_size = pred_item_ids.shape[0]
    pos_index = torch.zeros((batch_size, maxk), dtype=torch.bool)
    
    for i in range(batch_size):
        target_id = target_item_ids[i].item()
        for j in range(maxk):
            pred_id = pred_item_ids[i, j].item()
...
class MetricsCalculator:
    ...
    def update(
        ...
        labels: torch.Tensor,
        beam_size: int,
        pred_item_ids: torch.Tensor = None,
        target_item_ids: torch.Tensor = None
    ):
        ...
        Args:
            ...
            pred_item_ids: Optional predicted item IDs, shape (batch_size, beam_size)
            target_item_ids: Optional target item IDs, shape (batch_size,)
        """
        ...
        # Use item_id based comparison if available (more fair for dual-stream)
        if pred_item_ids is not None and target_item_ids is not None:
            pos_index = calculate_pos_index_by_item_id(pred_item_ids, target_item_ids, maxk=beam_size)
        else:
            # Fallback to code-based comparison
