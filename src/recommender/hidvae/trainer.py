...
class Trainer:
    ...
    def __init__(
        ...
        # Setup optimizer with special learning rate for fusion_alpha
        # Fusion alpha needs higher LR to learn effectively
        if hasattr(self.model, 'fusion_module') and hasattr(self.model.fusion_module, 'fusion_alpha'):
            # Separate fusion_alpha from other parameters
            fusion_alpha_params = [self.model.fusion_module.fusion_alpha]
            other_params = [p for p in self.model.parameters() if p is not self.model.fusion_module.fusion_alpha]
            
            self.optimizer = optim.Adam([
                {'params': other_params, 'lr': self.learning_rate, 'weight_decay': self.training_config.weight_decay},
                {'params': fusion_alpha_params, 'lr': self.learning_rate * 10, 'weight_decay': 0.0}  # 10x LR, no weight decay
            ])
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            if (epoch + 1) % self.training_config.eval_every_n_epochs == 0:
                ...
                if (epoch + 1) % self.training_config.save_every_n_epochs == 0:
                    self.save_checkpoint(valid_metrics, is_best=is_best)
