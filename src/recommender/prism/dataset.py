...
def load_purified_embeddings(content_path: str, collab_path: str) -> Tuple[Dict[int, np.ndarray], Dict[int, np.ndarray]]:
...
def create_datasets(
    ...
    if use_multimodal and purified_content_path and purified_collab_path:
        ...
        purified_content_dict, purified_collab_dict = load_purified_embeddings(
    ...
    valid_dataset = GenRecDataset(str(data_dir / "valid.parquet"), mode='evaluation', **dataset_kwargs)
    test_dataset = GenRecDataset(str(data_dir / "test.parquet"), mode='evaluation', **dataset_kwargs)
