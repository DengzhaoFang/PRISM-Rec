"""
Multi-Modal Dataset for HID-VAE Training
...
class HIDVAEDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"  Tag levels: {self.max_tag_level}")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        Returns:
            data_dict: Dictionary containing:
                ...
                - tag_ids: Mapped class indices at each level (padded to max_level)
        ...
        # Determine padding length (use first 3 layers: L2, L3, L4)
        # Truncate if more than 3 layers, pad if less
        max_layers = 3  # We only use 3 RQ layers
        
        if num_tags > max_layers:
            # Truncate to max_layers
            global_tag_ids = global_tag_ids[:max_layers]
            num_tags = max_layers
...
def create_dataloaders(
    ...
    max_items: Optional[int] = None,
    **dataset_kwargs
) -> Tuple[torch.utils.data.DataLoader, HIDVAEDataset]:
    """
    Create dataloader for HID-VAE training.
    ...
    Args:
        ...
        **dataset_kwargs: Additional arguments for HIDVAEDataset
        
    Returns:
        dataloader: DataLoader for training
        dataset: Dataset instance (for accessing metadata)
    """
    dataset = HIDVAEDataset(
        data_dir=data_dir,
        max_items=max_items,
        **dataset_kwargs
    )
...
def collate_hidvae_batch(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    """
    Custom collate function for batching HID-VAE data.
